...
public class Utils
{
    public static bool IsValidPassword(string password)
        ...
        /**
         * Generates a secure crypto random number and returns it in base 64 format.
         * TL;DR; The idea is to generate a number/text that is (impossible) to
         * be predicted by an human and by machines.
         * The generated value can be used as a token for various use cases.
         * The strongness is proportional to the number of bytes.
         */
        ...
        /*
         *  returns a string with the first char in uppercase and the remaining chars in lowercase
         */
        public static string CapitalizedWord(string value)
        {
            return value == null ? "" : $"{value.Substring(0, 1).ToUpper()}{value.Substring(1).ToLower()}";
        }


        public static T GetEnumFromStringWithSpaces<T>(string value) where T : struct
        {
            var statusStringArray = value.Split(" ");
            var statusValue = "";
            foreach (var s in statusStringArray) statusValue += CapitalizedWord(s);
            return Enum.Parse<T>(statusValue);
        }
